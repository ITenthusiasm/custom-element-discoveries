...
5. [x] ✅ When a **Developer** (or the component) programmatically deselects an `option` with **`option.selected = false`**:
   - If the `combobox` accepts the user's current filter as a `value` and it is not an empty string (i.e., the `combobox` is in `anyvalue` mode):
     - [x] ✅ **Do nothing**. The `combobox` has already been set to a valid value previously, so there is nothing to do.
   - Otherwise (i.e., if the `combobox` _does not_ accept the current filter as a `value` and/or the filter _is_ an empty string):
     - If the `combobox` accepts an Empty String as a valid value _and_ filter (i.e., the `combobox` is in `clearable` mode):
...
10. [x] ✅ When the `combobox` is initialized by the `SelectEnhancer` (**Component** action):
