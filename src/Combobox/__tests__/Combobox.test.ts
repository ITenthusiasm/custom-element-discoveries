...
const testConfigs = Object.freeze([
  { mode: "Regular", zmode: "regular", label: "Regular" },
  { mode: "Filterable", zmode: "filter", label: "Filterable" },
] as const);
...
for (const { mode } of testConfigs) {
  it.describe(`Combobox Web Component (${mode})`, () => {
    ...
    async function expectOptionToBeSelected(page: Page, { label, value }: OptionInfo, selected = true): Promise<void> {
      ...
      const option = page.getByRole("option", { name: label, includeHidden: true });
    ...
    if (mode === "Regular") {
      ...
      for (const filtertype of ["clearable", "anyvalue"] as const) {
      ...
      it.describe("Mouse Interactions", () => {
        ...
        if (mode === "Filterable") {
          it("Places the cursor in the right location when clicked", async ({ page }) => {
            ...
            // Verify that the `click` placed the cursor (`selection`) in the expected location.
            await combobox.evaluate((node: ComboboxField, offset) => {
              const text = node.firstChild as Text;
              ...
              if (
                ...
                throw new Error(`Expected combobox text to be selected at offset "${offset}"`);
              }
            }, cursorOffset);
          });
        }
      ...
      it.describe("Keyboard Interactions", () => {
          ...
          it("Selects all of its text content when focused by keyboard navigation", async ({ page }) => {
            ...
            await combobox.evaluate((node: ComboboxField) => {
              ...
              if (
                ...
                selection.focusNode !== selection.anchorNode ||
                selection.focusOffset !== text.length
              ) {
                throw new Error("Expected `combobox`'s enter text contents to be selected");
              }
            });
          });
        }
        ...
        it.describe("Escape", () => {
          it("Hides the `option`s", async ({ page }) => {
            ...
            await page.keyboard.press("Backspace"); // Reset filter if in Filter mode. Does nothing in Regular mode.
            await expectOptionsToBeVisible(page);
            await expectOptionToBeActive(page, { label: initialValue });
        ...
        it.describe("Enter", () => {
          ...
          /**
           * Associates the provided `combobox` with the (first) form element on the same page for testing. The
           ...
           */
          async function associateComboboxWithForm(
            combobox: Locator,
            association: "implicit" | "explicit",
          ): Promise<void> {
            await expect(combobox).toHaveRole("combobox");
            return combobox.evaluate(
              (field: ComboboxField, [assoc, attr, id]) => {
                const container = field.closest("select-enhancer") as SelectEnhancer;
                if (assoc === "explicit") field.setAttribute("form", id);
                ...
                form.insertAdjacentElement(assoc === "explicit" ? "beforebegin" : "afterbegin", container);
              },
          ...
          it("Submits the owning form if the `combobox` is collapsed", async ({ page }) => {
            ...
            const combobox = page.getByRole("combobox");
            await associateComboboxWithForm(combobox, "implicit");
            ...
            await associateComboboxWithForm(combobox, "explicit");
        ...
        if (mode === "Regular") {
          ...
          it.describe("Filtering Functionality (via Printable Characters)", () => {
            /*
             * TODO: Write ALL of these pending tests:
             * 1) Should we verify that `SpaceBar` can be used for filtering? YES!!! (Verify selection DOESN'T occur.)
             *
             * 2) Test re-showing ALL `option`s whenever expansion happens, then filtering as user types.
             *
             * 3) Displaying "No Options" when there is no match
             * 3a) The `emptyMessage` property (belongs in API Section)
             *
             * 4) Proper keyboard navigation when a filter is active
             */
